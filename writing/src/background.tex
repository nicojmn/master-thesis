...
New kind of hardware components have emerged, \acrfull{ssd} are replacing traditional \acrfull{hdd}  
...
Split stacks are networking stacks divided into multiple planes, each optimized for an architectural task. Recent state-of-the-art research uses this design to solve encountered bottlenecks () rather than offloading every operation. A common design is to separate control operations from data operations. This is the case for IO-TCP \cite{kim_rearchitecting_2023}, which splits \textbf{mTCP} \cite{jeong_mtcp_2014}, a userspace stack following this approach. The control plane is responsible for connection management, congestion control, and retransmission. It runs on the host \acrshort{cpu}, while the data plane is responsible for packet preparation and transfer, running on the smartNIC. This design prevents disk \acrshort{io} pollution in the \acrshort{cpu} cache and reduces contention, notably on the \acrfull{llc} miss rate which is improved by 27 \%. This allows a single core to saturate several \acrshort{nvme} disks at 77.4 Gbps, whereas Linux and Atlas \cite{marinos_diskcryptnet_2017} (a video streaming FreeBSD server) achieve roughly half that throughput even when using 10 cores. Network performance also benefits from reduced \acrshort{rtt} and improved throughput.

While IO-TCP addresses host-centric bottlenecks with a specialized userspace implementation, ZeroNIC \cite{skiadopoulos_high-throughput_2024} relies on unmodified Linux kernel stack for the control plane to maintain standards robustness, while its hardware data path entirely bypasses host memory using zero-copy and \acrshort{p2pdma} mechanisms previously described.
...
This master's thesis will focus on fully disaggregated datacenter, especially on interaction between \acrshort{nvme} storage pools and smartNIC.
...
\begin{draftbox}
	Pourquoi travailler sur IO-TCP et pas sur ZeroNIC ? 
	\begin{itemize}
		\item SPDK runs en user-space, donc c'est plus cohérent de se tourner vers une userspace stack plutôt que le kernel.
		\item Les auteurs de IO-TCP disent clairement que ext4 est pas optimal, et que SPDK est meilleur. 
		\item On utilise num\_p2p\_queues pour IO-TCP, donc à défault d'avoir la flexibilité de ZeroNIC, on a quand même le direct device access.
	\end{itemize}
\end{draftbox}


\section{IO-TCP implementation analysis}
