...
static void read_complete(struct spdk_bdev_io *bdev_io, bool success,
  ...
  uint64_t delta = end_tsc - hello_context->start_tsc;
...
static void hello_read(void *arg) {
  ...
  hello_context->start_tsc = spdk_get_ticks();
...
static void hello_start(void *arg1) {
  ...
  hello_context->buff =
      spdk_dma_zmalloc(hello_context->buff_size, buf_align, NULL);
  ...
  snprintf(hello_context->buff, hello_context->buff_size, "%s",
           "Hello World!\n");
