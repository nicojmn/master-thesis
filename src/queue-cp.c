...
struct context_t {
  ...
  struct io_task_t *tasks;
  TAILQ_HEAD(, io_task_t) retry_queue;
};
...
static void write_file(void *arg) {
  ...
  ctx->io_chunk = ctx->buf_size / g_opts.queue_depth;
  ...
  for (int i = 0; i < g_opts.queue_depth; i++) {
    struct io_task_t *task = &ctx->tasks[i];
    task->ctx = ctx;
    task->id = i;
    task->offset = i * ctx->io_chunk;
    rc = spdk_bdev_write(ctx->bdev_desc, ctx->bdev_io_channel,
                         ctx->buf + task->offset, task->offset, ctx->io_chunk,
                         write_complete, ctx);
    if (rc == -ENOMEM) {
      SPDK_NOTICELOG("Queuing I/O\n");
      ctx->bdev_io_wait.bdev = ctx->bdev;
...
static void discover(void *arg1) {
  ...
  ctx->bdev = NULL;
  ctx->bdev_desc = NULL;
  ctx->reads_completed = 0;
  ctx->writes_completed = 0;
  int rc;
  ...
  ctx->tasks = calloc(g_opts.queue_depth, sizeof(struct io_task_t));
  if (ctx->tasks == NULL) {
    SPDK_ERRLOG("Calloc tasks array\n");
    spdk_put_io_channel(ctx->bdev_io_channel);
    spdk_bdev_close(ctx->bdev_desc);
    spdk_app_stop(-1);
  }
...
static int parse_arg(int ch, char *argv) {
  ...
  case 'o':
    ...
    break;
  case 'q':
    g_opts.queue_depth = atoi(strdup(argv));
    break;
  default:
    usage();
    return -1;
...
int main(int argc, char **argv) {
  ...
  rc = spdk_app_parse_args(argc, argv, &opts, "f:o:q:", NULL, parse_arg, usage);
  if (rc == SPDK_APP_PARSE_ARGS_FAIL) {
    SPDK_ERRLOG("Error parsing args : %d\n", spdk_strerror(-rc));
    exit(rc);
  }
